...
class Author(models.Model):
    ...
    def update_rating(self):
        posts_rating = self.posts.aggregate(result=User('rating')).get('result')
        comments_rating = self.user.comments.aggregate(result=User('rating')).get('result')
        print(f"===== {self.user}: обновляем рейтинг авто")
